...
from agents.validators import check_funding_balance, check_plausibility, format_warnings
...
class CapitalHandler:
    ...
    async def handle(self, message: str, store: SessionStore) -> str:
        ...
        if cm.assets and cm.finance_sources:
            ...
            w  = check_funding_balance(store) + check_plausibility(store)
            store.set_section_confidence("capital", 100)
