...
from agents.validators import find_duplicates, check_plausibility, bep_warning, format_warnings
...
class RevenueHandler:
    ...
    async def handle(self, message: str, store: SessionStore) -> str:
        ...
        if rv.products and rv.year1_utilization > 0:
            ...
            w  = find_duplicates(store) + check_plausibility(store)
            store.set_section_confidence("revenue", 100)
            # Mark revenue as provisional since it was entered early
            if store.current_section != "review":
                store.mark_provisional("revenue_prices")
