...
from agents.validators import check_wc_gap, check_moratorium_vs_impl, bep_warning, quick_dscr_estimate, format_warnings
...
class WCHandler:
    ...
    async def handle(self, message: str, store: SessionStore) -> str:
        ...
        if wc.is_complete:
            ...
            w  = check_wc_gap(store) + check_moratorium_vs_impl(store) + bep_warning(store)
            store.set_section_confidence("wc", 100)
