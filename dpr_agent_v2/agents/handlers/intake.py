...
from agents.validators import normalize_currency, infer_entity_type, detect_implied_flags
...
class IntakeHandler:
    ...
    async def handle(self, message: str, store: SessionStore) -> str:
        ...
        """
        # A2: Normalize currency units before extraction
        norm_message = normalize_currency(message)

        # D16: Try entity type inference before LLM
        inferred_entity = infer_entity_type(message, store)

        prompts = [
            (PROFILE_PROMPT.format(msg=norm_message),   {}),
            (CAPITAL_PROMPT.format(msg=norm_message),   {"assets":[],"term_loans":[],"od_limit_lakhs":None,"promoter_equity_lakhs":None}),
            (REVENUE_PROMPT.format(msg=norm_message),   {"products":[],"year1_utilization":None,"working_days_per_month":None}),
            (COSTS_PROMPT.format(msg=norm_message),     {"raw_materials":[],"transport_base_lakhs":None,"misc_base_lakhs":None}),
            (MANPOWER_PROMPT.format(msg=norm_message),  {"categories":[]}),
            (WC_PROMPT.format(msg=norm_message),        {"debtor_days":None,"creditor_days_rm":None,"stock_days_rm":None,"implementation_months":None}),
        ]
        ...
        # D16: Apply entity inference if LLM missed it
        if inferred_entity and not profile_d.get("entity_type"):
            from core.session_store import EntityType
            try: store.project_profile.entity_type = EntityType(inferred_entity)
            except: pass
        ...
        # A5: Detect implied values + A2 flag scan on full message
        detect_implied_flags(message, store)
